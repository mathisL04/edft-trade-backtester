...
for selected_datetime in date_range:
    selected_date_str = selected_datetime.strftime('%d%b%y').upper()
    report_date_str = (selected_datetime + timedelta(days=1)).strftime('%d%b%y').upper()
    edft_label = selected_datetime.strftime('%Y-%m-%d')

    file_path_power = rf'C:\ZAINET_CLIENT\ODBS\MDR_ELEC_TRADES_{report_date_str}.csv'
    file_path_gas   = rf'C:\ZAINET_CLIENT\ODBS\MDR_GAS_TRADES_{report_date_str}.csv'

    print(f" Checking files for trade date {selected_date_str} → report file {report_date_str}...")

    if not os.path.exists(file_path_power) or not os.path.exists(file_path_gas):
        print(f"  Missing file(s) for {selected_date_str}. Skipping.")
        continue

    try:
        df_power = pd.read_csv(file_path_power)
        df_power['TRADE_DATE'] = pd.to_datetime(df_power['TRADE_DATE'], errors='coerce')
        df_power['TRADE_DATE'] = df_power['TRADE_DATE'].dt.strftime('%d/%m/%Y')

        df_power['TradeDate_EDFT_Label'] = selected_datetime.strftime('%Y-%m-%d')
        all_power.append(df_power)

        df_gas = pd.read_csv(file_path_gas)
        df_gas['TRADE_DATE'] = pd.to_datetime(df_gas['TRADE_DATE'], errors='coerce')
        df_gas['TRADE_DATE'] = df_gas['TRADE_DATE'].dt.strftime('%d/%m/%Y')

        df_gas['TradeDate_EDFT_Label'] = selected_datetime.strftime('%Y-%m-%d')
        all_gas.append(df_gas)


    except Exception as e:
        print(f"   Error loading files for {selected_date_str}: {e}")
        continue
...
#try:
#    file_date = datetime.strptime(filename_date, "%d%b%y")
#except ValueError:
#    print("Could not parse the file date correctly.")
#    exit()

#if selected_datetime.weekday() in [5, 6]:
#    print("Selected trade date is a weekend. No trades expected.")
#    exit()

#if report_datetime.weekday() in [5, 6]:
#    print("File/report date falls on a weekend. No trade file expected.")
#    exit()



##from pandas.tseries.offsets import BDay  # <-- add at top with other imports
...
#  Replaced: ensure reference date is a business day (skip weekends)
#edft_reference_date = selected_datetime - BDay(2)
#edft_reference_date_str = edft_reference_date.strftime('%Y-%m-%d')
#edft_reference_date = selected_datetime
#edft_reference_date_str = edft_reference_date.strftime('%Y-%m-%d')
...
def remove_negative_volume_duplicates(df):
    ...
    # Create helper column for absolute volume
